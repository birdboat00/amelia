const convertDimToPx = (dimensions: { w: number, h: number }, ppi = 96): { w: number, h: number } => {
    return {
...
const convertDistance = (millimeter: number, ppi = 96): number => {
    // mm = ( pixels * 25.4 ) / ppi;
    // px = mm * ppi / 24.5;
...
export class Size {
     ...
     * @returns {Size}
     */
    static fromPx(width: number, height: number): Size {
     ...
     * @returns {Size}
     */
    static fromMillimeters(width: number, height: number): Size {
     ...
     * @returns {Size}
     */
    static fromInches(width: number, height: number): Size {
